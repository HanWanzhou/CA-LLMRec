...
class llm4rec(nn.Module):
    def __init__(
        ...
        if llm_model == 'opt':
            # 使用更小的OPT-1.3B模型，减少下载时间从9.96GB到~2.6GB
            self.llm_model = OPTForCausalLM.from_pretrained("facebook/opt-1.3b", torch_dtype=torch.float16, load_in_8bit=True, device_map=self.device)
            self.llm_tokenizer = AutoTokenizer.from_pretrained("facebook/opt-1.3b", use_fast=False)
            
            # 超快选项：如果还想更快，取消下面两行的注释，注释上面两行
            # self.llm_model = OPTForCausalLM.from_pretrained("facebook/opt-350m", torch_dtype=torch.float16, load_in_8bit=True, device_map=self.device)
            # self.llm_tokenizer = AutoTokenizer.from_pretrained("facebook/opt-350m", use_fast=False)
